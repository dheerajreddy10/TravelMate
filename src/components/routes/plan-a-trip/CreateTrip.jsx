...
function CreateTrip({ createTripPageRef }) {
  ...
  const generateTrip = async () => {
    ...
    const plannedDays = Number(formData.noOfDays)
    if (plannedDays > 5) {
      return toast.error("Please enter Trip Days less than 5");
    }
    if (plannedDays < 1) {
    ...
    try {
      ...
      console.log('error occured while genrating trip', error)
      setIsLoading(false);
      toast.dismiss();
  ...
  console.log('key',import.meta.env.VITE_GOOGLE_MAP_API_KEY)
  ...
  const handlePlaceSelect = (selectedPlace) => {
    ...
    setFormData((prevState) => ({ ...prevState, location: selectedPlace?.formatted_address }));
    ...
    <div ref={createTripPageRef} className="mt-10 text-center">
      <div className="text">
        ...
        <p className="opacity-90 mx-auto text-center text-md md:text-xl font-medium tracking-tight text-primary/80">
          Discover, explore, and enjoy—hassle-free!. <br />
          <span className="bg-gradient-to-b text-2xl from-blue-400 to-blue-700 bg-clip-text text-center text-transparent">
            Travel Mate
          </span>{" "}
          <br /> will personalize your travel itinerary so you can focus on making memories
        </p>
      </div>
            ...
            <DialogDescription>
              <span className="flex gap-2">
                <span className="text-center w-full opacity-90 mx-auto tracking-tight text-primary/80">
                  {user
                    ? "Logged In Securely to TravelMate with Google Authentication"
                    : "Sign In to TravelMate with Google Authentication Securely"}
                </span>
              </span>
              {user ? (
                ""
